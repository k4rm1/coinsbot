...
// regarde discord 
Connectdatabase(); //à partir de là
...
async function checkTokenStatus() {
  ...
  // normal ya aucun bot dans la db lol set à la main et test azy
  for (const data of tokensFromDatabase) {
    ...
    if (!activeClients[data.token] && !expired && data.Status !== '1') {
      ...
      if (expired) {
        ...
        if (!data.lastWarningTime || data.lastWarningTime < warningTime) {// c l'id des bots
          if (activeClients[data.token]) {
